import { Cpu, Zap, Plus, Menu } from 'lucide-react'
    ...
    <header className="fixed top-0 z-40 w-full border-b border-border bg-background/80 backdrop-blur-md">
      <div className="flex h-16 items-center justify-between px-4 md:px-6">
        <div className="flex items-center gap-3">
          <button
            onClick={onMenuClick}
            className="flex h-10 w-10 items-center justify-center rounded-lg text-muted-foreground hover:bg-secondary hover:text-foreground md:hidden"
        ...
        <div className="flex items-center gap-2 md:gap-4">
          <Link
            to="/add"
            className="flex items-center gap-2 rounded-lg bg-primary px-3 py-2 text-sm font-medium text-white shadow-lg shadow-primary/20 transition-all hover:shadow-primary/40 hover:brightness-110 md:px-4"
          >
            <Plus className="h-4 w-4" />
            <span className="hidden sm:inline">Add New</span>
          </Link>
          <div className="hidden items-center gap-2 rounded-full bg-secondary/50 px-4 py-2 lg:flex">
